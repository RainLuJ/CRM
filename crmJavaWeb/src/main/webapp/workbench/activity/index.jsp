...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
                    ...
                    success: function (data) {
                        ...
                        if (data.success) {
                            ...
                            */
                            ...
                            // 提交需要在每次调用模态窗口之前，将窗口中的数据刷新；但是修改操作就不需要了（每次修改之前都会铺数据）
                            $("#activityAddForm")[0].reset();    //将表单中已经提交的数据清空
            ...
            $("#updateBtn").click(function () {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        "id": $.trim($("#edit-id").val()),  //所有者
                        "owner": $.trim($("#edit-owner").val()),  //所有者
                        "name": $.trim($("#edit-name").val()),    //名称
            ...
            //为全选的复选框绑定事件，触发全选操作
            // attr是给dom元素设置属性，prop是给dom 已有的属性 设置 值
            $("#checkAll").click(function () {
                // this.checked：当前按钮是否被选中   【true/false】   ====>逻辑：全选影响单选
                $("input[name=singleCheck]").prop("checked", this.checked);
            })

            /*  这种做法是不行的，因为动态生成的元素，是不能够以普通绑定事件的形式来进行操作的
             ...
             */
            $("#activityBody").on("click", $("input[name=singleCheck]"), function () {
                // 逻辑：单选影响全选
                $("#checkAll").prop("checked", $("input[name=singleCheck]").length === $("input[name=singleCheck]:checked").length);

            })

            $("#delBtn").click(function () {
                var $delEles = $("input[name=singleCheck]:checked");
                ...
                if ($delEles.length === 0) {
                    ...
                    if (confirm("确定删除所选记录吗？")) {
                        ...
                        for (var i = 0; i < $delEles.length; i++) {
                            param += "id=" + $delEles[i].value;

                            if (i !== $delEles.length - 1) {
                                param += "&";
                            }
                        }
            ...
            $("#modifyModalBtn").click(function () {
                ...
                var $mChek = $("input[name=singleCheck]:checked");

                if ($mChek.length === 0) {
                    alert("请选择需要修改的元素");
                } else if ($mChek.length > 1) {
                    alert("只能选择一条记录进行修改");
                } else {
                    var id = $mChek.val();
                    ...
                    $.ajax({
                        ...
                        success : function (data) {
                            ...
                            var id = "${sessionScope.user.id}";
                            $.each(data.uList,function (i,val) {
        ...
        function pageList(pageNo, pageSize) {
            ...
            //每次调用分页铺数据，都应该将 全选框的√ 干掉
            $("#checkAll").prop("checked", false);

            //查询前，将隐藏域中保存的信息取出来，重新赋予到搜索框中（保证了在未点“查询”按钮之前，无法使用表格中的查询条件）
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data.dataList, function (i, n) {
                        ...
                        // 根据复选框来对选中元素进行操作
                        html += '<td><input type="checkbox" name="singleCheck" value="' + n.id + '"/></td>';
    ...
    <div style="width: 100%; position: absolute;top: 5px; left: 10px;">
        ...
        <div class="btn-toolbar" role="toolbar"
            ...
            <div class="btn-group" style="position: relative; top: 18%;">
                ...
                <button type="button" class="btn btn-primary" id="addModalBtn"><span
                        class="glyphicon glyphicon-plus"></span> 创建
                </button>
                <%--  data-toggle="modal" data-target="#editActivityModal" --%>
                <button type="button" class="btn btn-default" id="modifyModalBtn"><span
